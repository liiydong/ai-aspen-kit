# -*- coding: utf-8 -*-
"""v16：SEP 块端口改 M0-1（照 Aspen 示例约定）"""
...
OUT = r'D:\<化工工作区>\NA-Chemical-10000t_v16.bkp'
t = open(SRC, encoding='utf-8', errors='ignore').read()

# 1) FLOWSHEET：T-301 改 SEP + 端口统一 M0-1
...
if old_fs in t:
    ...
    print('FLOWSHEET 已按完整串替换')
else:
    t, n = re.subn(r'(BLKID = "T-301" BLKTYPE = )"EXTRACT"( MDLTYPE = )"Extract"', r'\1"SEP"\2"Sep"', t)
    # 端口：只改 T-301 那条 OUT/IN
    i = t.find('BLKID = "T-301"')
    j = t.find('\\\\', i)
    seg = t[i:j]
    seg = seg.replace('S-111" M1-2 "S-115" M2-3', 'S-111" M0-1 "S-115" M0-1')
    seg = seg.replace('S-113" M1-2 "S-114" M0-1', 'S-113" M0-1 "S-114" M0-1')
    t = t[:i] + seg + t[j:]
    print('FLOWSHEET 分段替换，类型替换 %d 次' % n)
print(repr(t[t.find('BLKID = "T-301"'):t.find('BLKID = "T-301"') + 240]))
